...
**All three services — dashboard, API, worker — are live and verified.** The dashboard is deployed on Vercel at [crm-system-dashboard-drab.vercel.app](https://crm-system-dashboard-drab.vercel.app), the API and worker are deployed on Railway, and all three are connected to a real Supabase project and a real Upstash Redis instance. See [`docs/phase-17/DEPLOYMENT_REPORT.md`](docs/phase-17/DEPLOYMENT_REPORT.md) for the full verification record, including two independent genuine authenticated end-to-end browser sessions against the live stack. Sentry error tracking is live for all three services (real DSNs, verified with genuine captured errors in each project). No production domains, payment provider, communication providers (WhatsApp/email/SMS final accounts), or AI provider credentials exist yet — see [`ROADMAP.md`](ROADMAP.md) for the phase each of these is scheduled in, and [`docs/DEPLOYMENT_AND_ENV.md`](docs/DEPLOYMENT_AND_ENV.md) for the full deployment model. Per an explicit user decision recorded in `ROADMAP.md`'s Phase 17 notes, this deployment is being treated as one production-track pass inside a single existing project set rather than a separate staging environment ahead of a separate production one — real production-readiness prerequisites (a backup/restore rehearsal, final provider credentials, an approved domain) remain unmet regardless of that labeling choice.
