...
are not photos; `scan_status` stays `pending` for every upload — no malware
scanning provider is configured or approved anywhere in `docs/TOOLS.md`,
so this phase does not pretend to scan (CLAUDE.md section 16), matching
`app.storage.validation`'s own documented gap.
"""
